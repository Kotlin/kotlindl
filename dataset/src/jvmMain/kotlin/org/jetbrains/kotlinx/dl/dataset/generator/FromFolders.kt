...
/**
 * This [LabelGenerator] is responsible for extracting labels from the names of folders
 * where the images of the appropriate class are located.
 *
...
public class FromFolders(public val mapping: Map<String, Int>) : LabelGenerator<File> {
    override fun getLabel(dataSource: File): Float {
        val label = mapping[dataSource.parentFile.name] ?: error(
            "The parent directory of ${dataSource.absolutePath} is ${dataSource.parentFile.name}. " +
                    "No such class name in mapping $mapping"
        )
