...
internal class TruncatedNormalTest {
    ...
    fun initialize() {
        ...
        expected[0][0] = -0.045539435f
        expected[0][1] = 0.03842017f
        expected[1][0] = 0.07207375f
        expected[1][1] = 0.12769456f

        val shape = Shape.make(2, 2)

        EagerSession.create().use { session ->
            val tf = Ops.create(session)
            val instance: TruncatedNormal<Float> = TruncatedNormal(12L)
            val operand: Operand<Float> =
                instance.initialize(FAN_IN, FAN_OUT, tf, shapeOperand(tf, shape), getDType(), "default_name")
