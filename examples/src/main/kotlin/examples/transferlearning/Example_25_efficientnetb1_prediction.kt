...
at examples.transferlearning.Example_19_efficientnetb1_predictionKt.main(Example_19_efficientnetb1_prediction.kt:55)
at examples.transferlearning.Example_19_efficientnetb1_predictionKt.main(Example_19_efficientnetb1_prediction.kt)
 */
/**
 ...
 */
fun main() {
    val jsonConfigFile = getefficientnetb1JSONConfigFile()
    ...
    model.use {
        ...
        val hdfFile = getefficientnetb1WeightsFile()

        it.loadWeights(hdfFile)

        for (i in 1..8) {
            val inputStream = Dataset::class.java.classLoader.getResourceAsStream("datasets/vgg240/image$i.jpg")
...
private fun getefficientnetb1JSONConfigFile(): File {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb1JSONModelPath = properties["efficientnetb1JSONModelPath"] as String

    return File(efficientnetb1JSONModelPath)
}

/** Returns .h5 file with model weights, saved from Keras 2.x. */
private fun getefficientnetb1WeightsFile(): HdfFile {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb1h5WeightsPath = properties["efficientnetb1h5WeightsPath"] as String

    return HdfFile(File(efficientnetb1h5WeightsPath))
}
