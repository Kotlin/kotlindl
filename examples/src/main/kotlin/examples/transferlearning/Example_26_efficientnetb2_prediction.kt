...
at examples.transferlearning.Example_19_efficientnetb2_predictionKt.main(Example_19_efficientnetb2_prediction.kt:55)
at examples.transferlearning.Example_19_efficientnetb2_predictionKt.main(Example_19_efficientnetb2_prediction.kt)
 */
/**
 ...
 */
fun main() {
    val jsonConfigFile = getefficientnetb2JSONConfigFile()
    ...
    model.use {
        ...
        val hdfFile = getefficientnetb2WeightsFile()

        it.loadWeights(hdfFile)

        for (i in 1..8) {
            val inputStream = Dataset::class.java.classLoader.getResourceAsStream("datasets/vgg260/image$i.jpg")
...
private fun getefficientnetb2JSONConfigFile(): File {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb2JSONModelPath = properties["efficientnetb2JSONModelPath"] as String

    return File(efficientnetb2JSONModelPath)
}

/** Returns .h5 file with model weights, saved from Keras 2.x. */
private fun getefficientnetb2WeightsFile(): HdfFile {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb2h5WeightsPath = properties["efficientnetb2h5WeightsPath"] as String

    return HdfFile(File(efficientnetb2h5WeightsPath))
}
