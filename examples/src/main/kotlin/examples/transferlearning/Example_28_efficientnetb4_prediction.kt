...
at examples.transferlearning.Example_19_efficientnetb4_predictionKt.main(Example_19_efficientnetb4_prediction.kt:55)
at examples.transferlearning.Example_19_efficientnetb4_predictionKt.main(Example_19_efficientnetb4_prediction.kt)
 */
/**
 ...
 */
fun main() {
    val jsonConfigFile = getefficientnetb4JSONConfigFile()
    ...
    model.use {
        ...
        val hdfFile = getefficientnetb4WeightsFile()

        it.loadWeights(hdfFile)

        for (i in 1..8) {
            val inputStream = Dataset::class.java.classLoader.getResourceAsStream("datasets/vgg380/image$i.jpg")
...
private fun getefficientnetb4JSONConfigFile(): File {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb4JSONModelPath = properties["efficientnetb4JSONModelPath"] as String

    return File(efficientnetb4JSONModelPath)
}

/** Returns .h5 file with model weights, saved from Keras 2.x. */
private fun getefficientnetb4WeightsFile(): HdfFile {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb4h5WeightsPath = properties["efficientnetb4h5WeightsPath"] as String

    return HdfFile(File(efficientnetb4h5WeightsPath))
}
