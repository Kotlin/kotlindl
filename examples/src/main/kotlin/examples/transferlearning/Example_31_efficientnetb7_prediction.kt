...
at examples.transferlearning.Example_19_efficientnetb7_predictionKt.main(Example_19_efficientnetb7_prediction.kt:55)
at examples.transferlearning.Example_19_efficientnetb7_predictionKt.main(Example_19_efficientnetb7_prediction.kt)
 */
/**
 ...
 */
fun main() {
    val jsonConfigFile = getefficientnetb7JSONConfigFile()
    ...
    model.use {
        ...
        val hdfFile = getefficientnetb7WeightsFile()

        it.loadWeights(hdfFile)

        for (i in 1..8) {
            val inputStream = Dataset::class.java.classLoader.getResourceAsStream("datasets/vgg600/image$i.jpg")
...
private fun getefficientnetb7JSONConfigFile(): File {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb7JSONModelPath = properties["efficientnetb7JSONModelPath"] as String

    return File(efficientnetb7JSONModelPath)
}

/** Returns .h5 file with model weights, saved from Keras 2.x. */
private fun getefficientnetb7WeightsFile(): HdfFile {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb7h5WeightsPath = properties["efficientnetb7h5WeightsPath"] as String

    return HdfFile(File(efficientnetb7h5WeightsPath))
}
