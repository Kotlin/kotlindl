...
at examples.transferlearning.Example_19_efficientnetb3_predictionKt.main(Example_19_efficientnetb3_prediction.kt:55)
at examples.transferlearning.Example_19_efficientnetb3_predictionKt.main(Example_19_efficientnetb3_prediction.kt)
 */
/**
 ...
 */
fun main() {
    val jsonConfigFile = getefficientnetb3JSONConfigFile()
    ...
    model.use {
        ...
        val hdfFile = getefficientnetb3WeightsFile()

        it.loadWeights(hdfFile)

        for (i in 1..8) {
            val inputStream = Dataset::class.java.classLoader.getResourceAsStream("datasets/vgg300/image$i.jpg")
...
private fun getefficientnetb3JSONConfigFile(): File {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb3JSONModelPath = properties["efficientnetb3JSONModelPath"] as String

    return File(efficientnetb3JSONModelPath)
}

/** Returns .h5 file with model weights, saved from Keras 2.x. */
private fun getefficientnetb3WeightsFile(): HdfFile {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb3h5WeightsPath = properties["efficientnetb3h5WeightsPath"] as String

    return HdfFile(File(efficientnetb3h5WeightsPath))
}
