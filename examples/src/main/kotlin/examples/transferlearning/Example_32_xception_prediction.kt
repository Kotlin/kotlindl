...
at examples.transferlearning.Example_19_xception_predictionKt.main(Example_19_xception_prediction.kt:55)
at examples.transferlearning.Example_19_xception_predictionKt.main(Example_19_xception_prediction.kt)
 */
/**
 ...
 */
fun main() {
    val jsonConfigFile = getxceptionJSONConfigFile()
    ...
    model.use {
        ...
        val hdfFile = getxceptionWeightsFile()
...
private fun getxceptionJSONConfigFile(): File {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val xceptionJSONModelPath = properties["xceptionJSONModelPath"] as String

    return File(xceptionJSONModelPath)
}

/** Returns .h5 file with model weights, saved from Keras 2.x. */
private fun getxceptionWeightsFile(): HdfFile {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val xceptionh5WeightsPath = properties["xceptionh5WeightsPath"] as String

    return HdfFile(File(xceptionh5WeightsPath))
}
