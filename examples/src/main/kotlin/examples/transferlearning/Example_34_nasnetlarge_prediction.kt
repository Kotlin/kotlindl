...
// TODO: write about my experience and unhandled enasnetlarge with strange layer names
/**
...
Enasnetlarge in thread "main" java.lang.IllegalArgumentEnasnetlarge: invalid name: 'conv1/conv_conv2d_kernel' does not match the regular expression [A-Za-z0-9.][A-Za-z0-9_.\-]*
...
at examples.transferlearning.Example_19_nasnetlarge_predictionKt.main(Example_19_nasnetlarge_prediction.kt:55)
at examples.transferlearning.Example_19_nasnetlarge_predictionKt.main(Example_19_nasnetlarge_prediction.kt)
 */
/**
 ...
 */
fun main() {
    val jsonConfigFile = getnasnetlargeJSONConfigFile()
    ...
    model.use {
        ...
        val hdfFile = getnasnetlargeWeightsFile()

        it.loadWeights(hdfFile)

        for (i in 1..8) {
            val inputStream = Dataset::class.java.classLoader.getResourceAsStream("datasets/vgg331/image$i.jpg")
...
private fun getnasnetlargeJSONConfigFile(): File {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val nasnetlargeJSONModelPath = properties["nasnetlargeJSONModelPath"] as String

    return File(nasnetlargeJSONModelPath)
}

/** Returns .h5 file with model weights, saved from Keras 2.x. */
private fun getnasnetlargeWeightsFile(): HdfFile {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val nasnetlargeh5WeightsPath = properties["nasnetlargeh5WeightsPath"] as String

    return HdfFile(File(nasnetlargeh5WeightsPath))
}
