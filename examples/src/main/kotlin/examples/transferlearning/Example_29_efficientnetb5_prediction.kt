...
at examples.transferlearning.Example_19_efficientnetb5_predictionKt.main(Example_19_efficientnetb5_prediction.kt:55)
at examples.transferlearning.Example_19_efficientnetb5_predictionKt.main(Example_19_efficientnetb5_prediction.kt)
 */
/**
 ...
 */
fun main() {
    val jsonConfigFile = getefficientnetb5JSONConfigFile()
    ...
    model.use {
        ...
        val hdfFile = getefficientnetb5WeightsFile()

        it.loadWeights(hdfFile)

        for (i in 1..8) {
            val inputStream = Dataset::class.java.classLoader.getResourceAsStream("datasets/vgg456/image$i.jpg")
...
private fun getefficientnetb5JSONConfigFile(): File {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb5JSONModelPath = properties["efficientnetb5JSONModelPath"] as String

    return File(efficientnetb5JSONModelPath)
}

/** Returns .h5 file with model weights, saved from Keras 2.x. */
private fun getefficientnetb5WeightsFile(): HdfFile {
    val properties = Properties()
    val reader = FileReader("data.properties")
    properties.load(reader)

    val efficientnetb5h5WeightsPath = properties["efficientnetb5h5WeightsPath"] as String

    return HdfFile(File(efficientnetb5h5WeightsPath))
}
