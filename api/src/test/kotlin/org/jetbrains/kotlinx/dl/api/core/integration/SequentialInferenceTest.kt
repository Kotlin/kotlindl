...
class SequentialInferenceTest {
    ...
    fun basicInference(@TempDir tempDir: Path?) {
        ...
        model.use {
            assertEquals(model.layers.size, 8)
            ...
            assertEquals(it.layers[5].getParams(), 376440)
            assertEquals(it.layers[6].getParams(), 10164)
            assertEquals(it.layers[7].getParams(), 850)
